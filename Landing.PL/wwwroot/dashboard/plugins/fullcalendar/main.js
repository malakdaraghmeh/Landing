...
var FullCalendar = (function (exports) {
    ...
    Permission to use, copy, modify, and/or ~/dashboardribute this software for any
    ...
    */
    ...
    var BASE_OPTION_REFINERS = {
        ...
        eventDragMin~/dashboardance: Number,
        expandRows: Boolean,
        height: identity,
        ...
        selectMin~/dashboardance: Number,
        selectable: Boolean,
    ...
    */
    ...
    function computeFallbackHeaderFormat(datesRep~/dashboardinctDays, dayCnt) {
        // if more than one week row, or if there are a lot of columns with not much space,
        // put just the day numbers will be in each cell
        if (!datesRep~/dashboardinctDays || dayCnt > 10) {
    ...
    var DayHeader = /** @class */ (function (_super) {
        ...
        DayHeader.prototype.render = function () {
            ...
            var _a = this.props, dates = _a.dates, dateProfile = _a.dateProfile, datesRep~/dashboardinctDays = _a.datesRep~/dashboardinctDays, renderIntro = _a.renderIntro;
            var dayHeaderFormat = this.createDayHeaderFormatter(context.options.dayHeaderFormat, datesRep~/dashboardinctDays, dates.length);
            return (createElement(NowTimer, { unit: "day" }, function (nowDate, todayRange) { return (createElement("tr", { role: "row" },
                renderIntro && renderIntro('day'),
                dates.map(function (date) { return (datesRep~/dashboardinctDays ? (createElement(TableDateCell, { key: date.toISOString(), date: date, dateProfile: dateProfile, todayRange: todayRange, colCnt: dates.length, dayHeaderFormat: dayHeaderFormat })) : (createElement(TableDowCell, { key: date.getUTCDay(), dow: date.getUTCDay(), dayHeaderFormat: dayHeaderFormat }))); }))); }));
        };
        return DayHeader;
    }(BaseComponent));
    function createDayHeaderFormatter(explicitFormat, datesRep~/dashboardinctDays, dateCnt) {
        return explicitFormat || computeFallbackHeaderFormat(datesRep~/dashboardinctDays, dateCnt);
    }
    ...
    */
    ...
    function renderMicroColGroup(cols, shrinkWidth) {
        ...
        HOWEVER, Chrome was getting messing up ~/dashboardributing the width to <td>/<th> elements with colspans.
    ...
    */
    var AutoScroller = /** @class */ (function () {
        ...
        AutoScroller.prototype.handleSide = function (edge, seconds) {
            ...
            var inv~/dashboardance = edgeThreshold - edge.~/dashboardance;
            var velocity = // the closer to the edge, the faster we scroll
             ((inv~/dashboardance * inv~/dashboardance) / (edgeThreshold * edgeThreshold)) * // quadratic
        ...
        AutoScroller.prototype.computeBestEdge = function (left, top) {
            ...
            for (var _i = 0, _a = this.scrollCaches; _i < _a.length; _i++) {
                ...
                var left~/dashboard = left - rect.left;
                var right~/dashboard = rect.right - left;
                var top~/dashboard = top - rect.top;
                var bottom~/dashboard = rect.bottom - top;
                // completely within the rect?
                if (left~/dashboard >= 0 && right~/dashboard >= 0 && top~/dashboard >= 0 && bottom~/dashboard >= 0) {
                    if (top~/dashboard <= edgeThreshold && this.everMovedUp && scrollCache.canScrollUp() &&
                        (!bestSide || bestSide.~/dashboardance > top~/dashboard)) {
                        bestSide = { scrollCache: scrollCache, name: 'top', ~/dashboardance: top~/dashboard };
                    }
                    if (bottom~/dashboard <= edgeThreshold && this.everMovedDown && scrollCache.canScrollDown() &&
                        (!bestSide || bestSide.~/dashboardance > bottom~/dashboard)) {
                        bestSide = { scrollCache: scrollCache, name: 'bottom', ~/dashboardance: bottom~/dashboard };
                    }
                    if (left~/dashboard <= edgeThreshold && this.everMovedLeft && scrollCache.canScrollLeft() &&
                        (!bestSide || bestSide.~/dashboardance > left~/dashboard)) {
                        bestSide = { scrollCache: scrollCache, name: 'left', ~/dashboardance: left~/dashboard };
                    }
                    if (right~/dashboard <= edgeThreshold && this.everMovedRight && scrollCache.canScrollRight() &&
                        (!bestSide || bestSide.~/dashboardance > right~/dashboard)) {
                        bestSide = { scrollCache: scrollCache, name: 'right', ~/dashboardance: right~/dashboard };
                    }
                }
            }
            return bestSide;
    ...
    - minimum ~/dashboardance required before dragging
    ...
    */
    var FeaturefulElementDragging = /** @class */ (function (_super) {
        ...
        function FeaturefulElementDragging(containerEl, selector) {
            ...
            _this.delay = null;
            _this.min~/dashboardance = 0;
            _this.touchScrollAllowed = true; // prevents drag from starting and blocks scrolling during drag
            _this.mirrorNeedsRevert = false;
            _this.isInteracting = false; // is the user validly moving the pointer? lasts until pointerup
            _this.isDragging = false; // is it INTENTFULLY dragging? lasts until after revert animation
            _this.isDelayEnded = false;
            _this.is~/dashboardanceSurpassed = false;
            ...
            _this.onPointerDown = function (ev) {
                if (!_this.isDragging) { // so new drag doesn't happen while revert animation is going
                    ...
                    if (_this.isInteracting && // not destroyed via pointerdown handler
                        ...
                        _this.startDelay(ev);
                        if (!_this.min~/dashboardance) {
                            _this.handle~/dashboardanceSurpassed(ev);
                        }
                    }
                }
            };
            _this.onPointerMove = function (ev) {
                if (_this.isInteracting) {
                    ...
                    if (!_this.is~/dashboardanceSurpassed) {
                        var min~/dashboardance = _this.min~/dashboardance;
                        var ~/dashboardanceSq = void 0; // current ~/dashboardance from the origin, squared
                        var deltaX = ev.deltaX, deltaY = ev.deltaY;
                        ~/dashboardanceSq = deltaX * deltaX + deltaY * deltaY;
                        if (~/dashboardanceSq >= min~/dashboardance * min~/dashboardance) { // use pythagorean theorem
                            _this.handle~/dashboardanceSurpassed(ev);
                        }
                    }
                    if (_this.isDragging) {
        ...
        FeaturefulElementDragging.prototype.handle~/dashboardanceSurpassed = function (ev) {
            this.is~/dashboardanceSurpassed = true;
            this.tryStartDrag(ev);
        };
        FeaturefulElementDragging.prototype.tryStartDrag = function (ev) {
            if (this.isDelayEnded && this.is~/dashboardanceSurpassed) {
    ...
    */
    var DateSelecting = /** @class */ (function (_super) {
        ...
        function DateSelecting(settings) {
            ...
            dragging.min~/dashboardance = options.selectMin~/dashboardance || 0;
    ...
    var EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                dragging.min~/dashboardance = ev.isTouch ? 0 : options.eventDragMin~/dashboardance;
                dragging.delay =
    ...
    var EventResizing = /** @class */ (function (_super) {
        ...
        function EventResizing(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                _this.dragging.min~/dashboardance = component.context.options.eventDragMin~/dashboardance;
    ...
    */
    var ExternalDraggable = /** @class */ (function () {
        function ExternalDraggable(el, settings) {
            ...
            this.handlePointerDown = function (ev) {
                var dragging = _this.dragging;
                var _a = _this.settings, min~/dashboardance = _a.min~/dashboardance, longPressDelay = _a.longPressDelay;
                dragging.min~/dashboardance =
                    min~/dashboardance != null ?
                        min~/dashboardance :
                        (ev.isTouch ? 0 : BASE_OPTION_DEFAULTS.eventDragMin~/dashboardance);
                dragging.delay =
    ...
    var InferredElementDragging = /** @class */ (function (_super) {
        ...
        function InferredElementDragging(containerEl) {
            ...
            _this.handlePointerDown = function (ev) {
                _this.emitter.trigger('pointerdown', ev);
                if (!_this.shouldIgnoreMove) {
                    // fire dragstart right away. does not support delay or min-~/dashboardance
                    _this.emitter.trigger('dragstart', ev);
                }
            };
    ...
    */
    ...
    var DayTableView = /** @class */ (function (_super) {
        ...
        DayTableView.prototype.render = function () {
            ...
            var headerContent = options.dayHeaders && (createElement(DayHeader, { ref: this.headerRef, dateProfile: props.dateProfile, dates: dayTableModel.headerDates, datesRep~/dashboardinctDays: dayTableModel.rowCnt === 1 }));
    ...
    */
    ...
    var DayTimeColsView = /** @class */ (function (_super) {
        ...
        DayTimeColsView.prototype.render = function () {
            ...
            var headerContent = options.dayHeaders && (createElement(DayHeader, { dates: dayTableModel.headerDates, dateProfile: dateProfile, datesRep~/dashboardinctDays: true, renderIntro: hasAttachedAxis ? this.renderHeadAxis : null }));
