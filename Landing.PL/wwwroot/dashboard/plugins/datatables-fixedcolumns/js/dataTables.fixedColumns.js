...
(function () {
    ...
    var FixedColumns = /** @class */ (function () {
            ...
            for (var i = 0; i < numCols; i++) {
                ...
                if (i < this.c.left) {
                    ...
                    if (i !== 0) {
                        ...
                        if (prevCol.visible()) {
                            ~/dashboardLeft += $(prevCol.nodes()[0]).outerWidth();
                        }
                    }
                    ...
                    for (var _i = 0, rows_1 = rows; _i < rows_1.length; _i++) {
                        ...
                        $($(row).children()[i - invisibles])
                            .css(this._getCellCSS(false, ~/dashboardLeft, 'left'))
                            .addClass(this.classes.fixedLeft);
                    }
                    // Add the css for the header and the footer
                    colHeader
                        .css(this._getCellCSS(true, ~/dashboardLeft, 'left'))
                        .addClass(this.classes.fixedLeft);
                    colFooter
                        .css(this._getCellCSS(true, ~/dashboardLeft, 'left'))
                        .addClass(this.classes.fixedLeft);
            ...
            for (var i = numCols - 1; i >= 0; i--) {
                ...
                if (i >= numCols - this.c.right) {
                    ...
                    if (i !== numCols - 1) {
                        var prevCol = this.s.dt.column(i + 1);
                        if (prevCol.visible()) {
                            ~/dashboardRight += $(prevCol.nodes()[0]).outerWidth();
                        }
                    }
                    ...
                    for (var _b = 0, rows_3 = rows; _b < rows_3.length; _b++) {
                        var row = rows_3[_b];
                        $($(row).children()[i + invisibles])
                            .css(this._getCellCSS(false, ~/dashboardRight, 'right'))
                            .addClass(this.classes.fixedRight);
                    }
                    // Add the css for the header and the footer
                    colHeader
                        .css(this._getCellCSS(true, ~/dashboardRight, 'right'))
                        .addClass(this.classes.fixedRight);
                    colFooter
                        .css(this._getCellCSS(true, ~/dashboardRight, 'right'))
                        .addClass(this.classes.fixedRight);
         ...
         * @param ~/dashboard The ~/dashboardance that the cell should be moved away from the edge
         * @param lr Indicator of fixing to the left or the right
         * @returns An object containing the correct css
         */
        FixedColumns.prototype._getCellCSS = function (header, ~/dashboard, lr) {
            if (lr === 'left') {
                return !this.s.rtl ?
                    {
                        left: ~/dashboard + 'px',
                    ...
                    {
                        position: 'sticky',
                        right: ~/dashboard + (header ? this.s.barWidth : 0) + 'px'
                    };
            }
            else {
                return !this.s.rtl ?
