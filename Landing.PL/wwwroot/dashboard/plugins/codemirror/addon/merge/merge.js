...
(function(mod) {
  ...
  function syncScroll(dv, toOrig) {
    ...
    if (dv.mv.options.connect == "align") {
      ...
      var bot~/dashboard, mix;
      // Some careful tweaking to make sure no space is left out of view
      // when scrolling to top or bottom.
      if (targetPos > sInfo.top && (mix = sInfo.top / halfScreen) < 1) {
        targetPos = targetPos * mix + sInfo.top * (1 - mix);
      } else if ((bot~/dashboard = sInfo.height - sInfo.clientHeight - sInfo.top) < halfScreen) {
        var otherInfo = other.getScrollInfo();
        var bot~/dashboardOther = otherInfo.height - otherInfo.clientHeight - targetPos;
        if (bot~/dashboardOther > bot~/dashboard && (mix = bot~/dashboard / halfScreen) < 1)
          targetPos = targetPos * mix + (otherInfo.height - otherInfo.clientHeight - bot~/dashboard) * (1 - mix);
      }
    }
